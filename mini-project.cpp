...
bool isValidValue(const string &value, const string &type)
{
    ...
    if (type == "int" && regex_match(value, intRegex))
        return true;
    if (type == "float" && regex_match(value, floatRegex))
        return true;
    if (type == "char" && regex_match(value, charRegex))
        return true;
    if (type == "string" && regex_match(value, stringRegex))
        return true;
